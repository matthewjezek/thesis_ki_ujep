...
This bachelor thesis focuses on the possibilities and technological challenges of integrating custom web services into the environment of dominant social networks, specifically the Facebook and X platforms. In today's digital space, these networks do not only serve for simple communication but are becoming complex ecosystems where connection with external applications is essential for maintaining user comfort and efficient content sharing. The thesis provides a detailed analysis of the current state of technological integration support, particularly OAuth authentication protocols and available API interfaces. The practical contribution is the development of the Sitzy pilot web application, which serves for carpooling management and demonstrates the optimal technology stack and security aspects of such a connection. The research part of the thesis utilizes an innovative methodology that, within a "Survey mode," automatically pairs telemetry data on real user behavior with their subjective evaluation in a questionnaire. The results offer a comprehensive view of the sustainability and risks of integrations in the dynamically changing environment of the API policies of these platforms.
...
Integrace vlastní webové aplikace se sociálními sítěmi není pouze technická disciplína. Z~pohledu marketingové strategie jde o~standardní součást budování tzv. \textit{owned}, \textit{earned} a~\textit{shared} mediálních kanálů značky či~produktu \cite{Quesenberry2024}. Realizace tohoto propojení však v~současném webovém prostředí naráží na zásadní překážku: dominantní sociální sítě se z~otevřených komunikačních nástrojů proměnily v~uzavřené ekosystémy, tzv. \textit{Walled Gardens}, které záměrně omezují interoperabilitu a~přenositelnost dat směrem k~nezávislým vývojářům \cite{Marro2025,DeNardis2015}.
...
Cílem práce je analýza dostupných možností integrace externích webových služeb do vybraných sociálních sítí a~návrh referenční bezheslové architektury, která tyto možnosti reflektuje. Práce se soustředí na porovnání technologické podpory u~sítí zaměřených na běžnou komunikaci a~sdílení obsahu (Facebook, X) a~alternativních komunitních platforem (Slack, Discord, Telegram), s~důrazem na bezpečnost delegované autentizace a~udržitelnost řešení nezávislého na jediném poskytovateli identity. Navržený koncept je následně ověřen na pilotní implementaci aplikace Sitzy, určené pro správu spolujízd a~vizualizaci obsazenosti míst v~automobilech, včetně vyhodnocení uživatelské zpětné vazby na skupině respondentů.

Pro zpracování práce byla zvolena inženýrská metodika Code-First, tedy vývoj a~testování funkčního proof-of-concept ještě před samotným sepsáním textu. Tento přístup umožnil ověřit proveditelnost navržené integrace v~praxi a~v~textu reflektovat i~konkrétní technické komplikace a~jejich řešení, se kterými se vývojáři při~integraci do proprietárních ekosystémů nevyhnutelně setkávají.

\chapter{Teoretická východiska integrace a autorizace na webu}

Předložená kapitola definuje teoretický rámec a provádí analýzu technologických 
standardů, protokolů a bezpečnostních paradigmat, která tvoří základní pilíře pro 
integraci externích webových služeb do ekosystémů sociálních sítí. Z marketingového 
hlediska tato integrace umožňuje transformaci vlastního (\textit{owned}) obsahu do 
sdílené (\textit{shared}) a získané (\textit{earned}) podoby v prostředí sociálních 
sítí \cite{Quesenberry2024}; z technického hlediska však vyžaduje robustní řešení 
delegované autorizace, jemuž je věnována zbývající část kapitoly. Pozornost je soustředěna na evoluci architektonických stylů a autorizačních mechanismů, které v současnosti determinují způsob bezpečného přenosu identit a řízení přístupových oprávnění v distribuovaném prostředí internetu.
...
\subsection{Implementace mechanismu PKCE (Proof Key for Code Exchange)}
\label{subsec:pkce_mechanismus}

K eliminaci rizika zachycení autorizačního kódu definuje standard RFC 7636 \cite{SakimuraEd2015} mechanismus PKCE. Ten zavádí dynamické kryptografické provázání mezi fází inicializace autorizačního požadavku a fází výměny kódu na token endpointu.
...
\item \textbf{Výpočet} \path{code_challenge}: Klient provede transformaci verifikátoru. Standard striktně nařizuje jako povinnou k implementaci (MTI) metodu S256, která využívá SHA-256 hashování \cite{SakimuraEd2015}: 
\[
...
Útočník, který by autorizační kód zachytil, nedisponuje znalostí původního verifikátoru (pouze jeho jednosměrného hashe), což mu znemožňuje úspěšné dokončení toku na token endpointu \cite{SakimuraEd2015}.

\section{Bezpečnostní standardy dle RFC 9700 (leden 2025)}

\subsection{Ochrana přesměrovacích toků}

V lednu 2025 vydala pracovní skupina IETF aktualizovaný standard RFC 9700 \cite{Lodderstedt2025}, který zpřísňuje požadavky na implementace OAuth 2.0. Jedním z klíčových bodů je striktní zákaz volného porovnávání přesměrovacích URI. Autorizační servery musí vyžadovat přesnou shodu řetězců (Exact String Matching) s registrovanými adresami \cite{Lodderstedt2025}. Použití zástupných znaků či regulárních výrazů je zakázáno, neboť v minulosti umožňovalo útočníkům obejít validaci a přesměrovat kód na vlastní subdomény.
...
Na základě těchto parametrů byly pro srovnání zvoleny tři open-source projekty představující odlišné architektonické přístupy a dva akademické projekty, které se soustředí na specifické algoritmické a funkční aspekty.
...
\begin{description}
    ...
    \item[Carpooling-Mobile-App] Prvním zástupcem je mobilní aplikace \textit{Carpooling-Mobile-App} \cite{Raut2016}. Tento projekt z roku 2016, tedy z období předcházejícího širší standardizaci a implementaci mechanismu PKCE, představoval nativní klientskou aplikaci pro operační systém Android napsanou v jazyce Java. Z hlediska integrace sociálních sítí byla v systému implementována možnost přihlášení pomocí účtů Facebook a Google, které však plnily pouze funkci doplňkového autentizačního kanálu k tradiční registraci založené na kombinaci uživatelského jména a hesla. Celá architektura se opírala o cloudovou platformu Backendless, reprezentující model \textit{Backend-as-a-Service} (BaaS), tedy cloudový model, kde vývojář outsourcuje veškerou backendovou logiku třetí straně a soustředí se pouze na frontend \cite{Okanda2024}. Přestože nasazení hotového cloudového rozhraní urychlilo počáteční fázi vývoje, řešení vykazovalo zásadní nedostatky v podobě silné technologické závislosti na poskytovateli a omezené kontroly nad životním cyklem uživatelských sezení. Absence hlubší integrace obsahu se projevovala zejména při sdílení jízd, které probíhalo formou prostého kopírování textových řetězců bez optimalizace pro mobilní prohlížeče či sociální náhledy.

    \item[icare] Otevřená platforma \textit{icare} od vývojářské skupiny diowa \cite{Icare2025} využívá webový framework Ruby on Rails v kombinaci s perzistentní vrstvou PostgreSQL a asynchronním zpracováním úloh. Vzhledem k původnímu určení systému jako technologického základu pro komerční produkt Company Carpool představuje toto řešení relevantní ekvivalent k implementacím určeným pro uzavřené komunity. Autentizační mechanismy byly v průběhu vývoje delegovány na externí službu Auth0 (vzor \textit{buy}), zatímco pro generování sociálních náhledů projekt využívá neproprietární Open Graph meta tagy nezávislé na Facebook SDK\footnote{Aktivní funkce přímého sdílení na Facebook byla z bezpečnostních a architektonických důvodů odstraněna ve verzi 0.61.0; tento stav přetrvává i v aktuálním tagu 0.115.0 z roku 2022.}. Z architektonického hlediska však tato delegace vytváří kritickou závislost na externím subjektu, což u~nízkonákladových projektů představuje riziko spojené s finanční náročností při škálování uživatelské základny.

    \item[Caroster] Srovnávací analýzu uzavírá platforma \textit{Caroster} \cite{Caroster2026}. Architektura systému je striktně rozdělena na klientskou část v Next.js a headless redakční systém Strapi na backendu. Projekt je koncipován jako eventová platforma, kde se koordinace spolujízdy váže na konkrétní události. Distribuce pozvánek probíhá prostřednictvím staticky generovaných URL adres. I~přes moderní uživatelské rozhraní je integrace do mobilních ekosystémů omezena absencí podpory rozhraní Web Share API. Z hlediska rozšiřitelnosti využívá Caroster pluginovou architekturu systému Strapi a pro geokódování se spoléhá na Mapbox API, což sice usnadňuje vývoj, ale zvyšuje technologický dluh a závislost na externích klíčích. Z bezpečnostního hlediska však dostupná dokumentace neuvádí podporu pokročilých standardů pro ochranu autorizačních toků, jako je PAR. Tato architektonická mezera v kombinaci se spoléháním na tradiční SMTP notifikace namísto přímé integrace sociálních API činí aplikaci zranitelnou v~kontextu moderních hrozeb cílících na webové služby.
...
\begin{description}
    ...
    \item[Hashemite University] Systém pro real-time spolujízdu vyvinutý na 
    Hashemite University \cite{Salhi2019} využívá Firebase Realtime Database pro 
    okamžitou synchronizaci souřadnic a implementuje optimalizovaný vyhledávací 
    algoritmus $k$-Nearest Neighbors ($k$-NN) pro automatické párování řidičů 
    a cestujících na shodné trase. Z hlediska integrace je však systém izolovaný, 
    spoléhá na proprietární datové struktury Firebase a neřeší otázky bezpečnosti 
    federované autentizace ani legislativní požadavky na ochranu osobních údajů.

    \item[UTAR Kampar] Carpoolingová platforma pro studenty univerzity UTAR Kampar 
    \cite{Tan2025} integruje konverzační rozhraní Google Dialogflow jako 
    inteligentního chatbota pro vyhledávání volných jízd a využívá knihovnu 
    PaddleOCR pro automatizované vytěžování rozvrhů hodin z obrázků, což usnadňuje 
    automatickou tvorbu pravidelných jízd. Aplikace je napsána ve frameworku 
...
V oblasti distribuce pozvánek představuje významný technologický precedens řešení platformy Slack využívající bezheslové přihlašovací odkazy (Magic Links) \cite{MagicLinkIF}. Tento přístup kombinuje registraci uživatele, ověření identity a okamžité přidělení přístupových práv do jediné uživatelské akce. V aplikaci Sitzy je tento koncept adaptován pro distribuci pozvánek, kde odkaz vygenerovaný řidičem slouží k okamžitému a bezpečnému zapsání cestujícího na volné sedadlo bez nutnosti zdlouhavého nastavování hesel.

\section{Zhodnocení silných a slabých stránek existujících přístupů}
\label{sec:zhodnoceni_pristupu}

Na základě provedené rešerše byla sestavena srovnávací \vref{tab:srovnani_projektu}, která staví vedle sebe vybrané existující projekty a nově navrženou aplikaci Sitzy.
...
Z těchto důvodů se návrh vlastní architektury aplikace Sitzy opírá o vývoj dedikovaného a~plně kontrolovaného asynchronního backendu na bázi FastAPI, který umožňuje implementovat pokročilé zabezpečení (OAuth 2.0 s PKCE a PAR, oddělené uložení sezení v PostgreSQL s vazbou dual-FK) s nulovými provozními licenčními poplatky, čímž představuje ideální technologickou odpověď pro komunitní spolujízdu v uzavřených skupinách.

\chapter{Analýza integračního potenciálu vybraných platforem a platformních rizik}
...
\begin{description}
    \item[Administrátoři (Administrators)]: představují nejvyšší úroveň autority. Disponují absolutním přístupem k nastavení, mohou resetovat tajný klíč aplikace (App Secret) nebo měnit role ostatních uživatelů. 
...
Aktivace tohoto oprávnění je však v konzoli vývojáře podmíněna deklarací transparentnosti. Vývojář je povinen v nastavení aplikace uvést validní URL adresy směřující na \UV{Zásady ochrany osobních údajů} a \UV{Smluvní podmínky} \cite{XDev2026}. Tato podmínka představuje pro nezávislé projekty nutnost vytvoření právního rámce již v rané fázi vývoje. Z hlediska architektury aplikace \textit{Sitzy} to znamená, že ačkoliv je e-mail dostupný, systém musí být navržen tak, aby primární identitu vázal na unikátní \path{social_id}, neboť uživatel může sdílení e-mailu v rámci autorizačního dialogu odmítnout \cite{XAPI20251110}.
...
Discord představuje ideální prostředí pro koordinaci aktivit v komunitách. Portál nabízí dva hlavní mechanismy:
...
\begin{itemize}
    \item \textbf{Meta (Facebook):} Dokumentační dluh a rigidní správa rolí (podrobně viz \vref{subsec:sprava_uzivatelskych_roli_v_testovacim_rezimu}). Absence přístupu k sociálnímu grafu pro nezávislé vývojáře znemožňuje nativní integraci seznamů přátel.
    
    \item \textbf{X (Twitter):} Model pay-per-use s kreditním systémem přenáší ekonomické riziko přímo na vývojáře (detailní analýza v \vref{subsec:urovne_pristupu_a_implementace_pkce}). Asymetrický sociální graf je pro plánování spolujízd méně relevantní než přímé kontakty.
...
\toprule
\textbf{Parametr} & \textbf{Meta (Facebook)} & \textbf{X (Twitter)} & \textbf{Slack / Discord} & \textbf{Telegram} \\ 
\midrule
...
Relační model se skládá z deseti klíčových entit:
...
\item \textbf{IntegrationAuditLog (\path{integration_audit_logs})}: Slouží k auditování kritických integračních událostí a bezpečnostních změn v datovém formátu JSON.
...
\chapter{Realizace, zajištění kvality a nasazení systému}
...
Rozsah oprávnění byl v souladu se zásadou minimalizace dat omezen na úroveň \textit{Read}, která postačuje k získání unikátního identifikátoru a základních profilových údajů. Pro přístup k~e-mailové adrese skrze scope \path{users.email} bylo nutné aktivovat volbu \textit{Request email from users}, která podmiňuje funkčnost integrace uvedením validních odkazů na zásady ochrany soukromí a smluvní podmínky. Tato administrativa představuje pro nezávislé projekty nutnost vytvoření právního rámce již v rané fázi vývoje. Konfigurace oprávnění je zobrazena v \extpriloha{externí příloze}{https://github.com/matthewjezek/thesis_ki_ujep/blob/fd77609d0333b87c656832fa3ce48e46de372957/images/x_app_permissions.png}{výřez z X Developer Console (App permissions)}.
...
Fyzická struktura repozitáře a logické rozdělení odpovědností jsou znázorněny v následujícím hierarchickém schématu:
...
\subsection{Bezpečné řízení relací a asynchronní správa stavu} \label{sec:rizeni_relaci}
...
Během externího testování na mobilních zařízeních byla odhalena specifická chybová anomálie u uživatelů využívajících platformy se zvýšenou ochranou soukromí. Konkrétně se jednalo o~mobilní telefon Pixel s operačním systémem GrapheneOS a výchozím prohlížečem Vanadium (který představuje privacy-first Chromium fork). Tento prohlížeč má z bezpečnostních důvodů ve výchozím nastavení zcela deaktivovanou JIT (\textit{Just-In-Time}) kompilaci JavaScriptu.
...
Jelikož backend v souladu se standardy striktně uplatňuje jednorázovost stavových tokenů a po prvním přečtení klíč z paměti Redis okamžitě maže (viz atomická operace \path{getdel} v \textbf{ukázce kódu \vref{lst:redis_getdel}}), první z těchto požadavků uspěl, avšak druhý okamžitě narazil na již neexistující stav a vrátil HTTP chybu \texttt{400 Bad Request}. Tato chybová odpověď následně na klientovi přepsala stav úspěšného přihlášení a uživatele přesměrovala zpět na přihlašovací obrazovku.
...
prohlížečům nainstalovaným přímo v CI runneru. Zvláštní krok \path{a11y:ci} je věnován 
kontrole přístupnosti; v aktuální podobě spouští statickou lintovací analýzu 
(\path{eslint-plugin-jsx-a11y}) s nulovou tolerancí varování, nikoliv runtime audit 
...
CI, konkrétně důsledným používáním atributů \path{aria-label}, \path{aria-hidden} a 
\path{aria-labelledby} u interaktivních prvků a dialogových oken, v souladu s doporučeními 
...
\chapter{Uživatelský výzkum a serverless telemetrie}
...
Předložená bakalářská práce úspěšně splnila všechny stanovené cíle. V~rámci teoretické i~praktické části byla detailně analyzována problematika integrace webových služeb do proprietárních ekosystémů sociálních sítí a~navrženo referenční řešení, které bylo úspěšně ověřeno v~reálném provozu na pilotní aplikaci Sitzy. Vývoj této platformy podle inženýrské metodiky \textit{Code-First} umožnil konfrontovat teoretické předpoklady s~praktickými limity a~vytvořit robustní, bezheslový systém plně respektující moderní bezpečnostní standardy i~legislativní požadavky na ochranu osobních údajů.

Zhodnocení klíčových architektonických rozhodnutí potvrzuje, že úplné vyloučení lokální správy hesel a~delegování autentizační vrstvy na prověřené poskytovatele identity představuje vysoce efektivní strategii. Tento přístup v~souladu s~principy ochrany soukromí již od návrhu zcela eliminoval riziko úniku citlivých přihlašovacích údajů a~hrozbu zneužití odcizených identit na jiných službách. Použití pokročilých standardů, jako je protokol OAuth 2.0 chráněný kryptografickým mechanismem pro veřejné klienty, a~bezpečné uložení sezení na backendu pomocí oddělené správy relací, prokázalo vysokou úroveň zabezpečení. Uložení obnovovacích tokenů v~zabezpečených klientských cookies s~nastavením pro zamezení přístupu skriptů a~ochranu před útoky napříč weby úspěšně izolovalo klientskou jednostránkovou aplikaci od citlivých operací. Z~hlediska legislativního souladu s~nařízením GDPR se jako zásadní ukázala referenční integrita relačního schématu, která při smazání účtu garantuje fyzickou a~kaskádovou likvidaci uživatelských dat přímo na úrovni databázového stroje, zatímco oddělené zpracování auditních logů bezpečně uchovává nezbytnou auditní stopu bez ukládání osobních identifikátorů.

Praktická realizace však zároveň odhalila slabé stránky a~rizika plynoucí z~vysoké závislosti na rozhraních třetích stran. Rigidní politiky a~nestabilita rozhraní v~prostředí uzavřených ekosystémů představují pro nezávislé vývojáře značné administrativní i~technické překážky. Během uživatelského výzkumu se jako největší bariéra projevil testovací režim platformy Meta, kde absence deklarované role pro běžné testery a~nutnost manuálního schvalování vývojářských účtů pro každého respondenta drasticky zvýšily uživatelské tření při onboardingu a~omezily využití přihlášení přes Facebook na minimum. Naopak decentralizovaný systém distribuce pozvánek prostřednictvím jednorázových odkazů se ukázal jako mimořádně úspěšná strategická odpověď na tato omezení. Tím, že se propojování uživatelů přesunulo mimo uzavřené sociální sítě do autonomní správy aplikace, byla zachována plná nezávislost na vnitřních sociálních grafech externích platforem.\\

Pro další rozvoj a~zvýšení provozní odolnosti systému se jako nejvhodnější krok jeví integrace dalších významných poskytovatelů federované identity, konkrétně služeb Google a~Apple. Přestože se v~těchto případech nejedná o~klasické sociální sítě, jejich zapojení do autentizační vrstvy představuje klíčové inženýrské opatření pro zajištění nezbytné redundance. Diverzifikace přihlašovacích autorit na stabilní ekosystémy Google a~Apple by účinně eliminovala závislost na úzkém portfoliu třetích stran, zmírnila platformní rizika spojená s~častými změnami politik poskytovatelů a~zároveň by dramaticky usnadnila registraci na mobilních zařízeních s~operačními systémy iOS a~Android.

Z~hlediska dlouhodobé technologické perspektivy by mohl systém v~budoucnu směřovat k~nativní bezheslové autentizaci pomocí otevřených standardů a~přihlašovacích klíčů (Passkeys). Toto řešení by kombinovalo maximální uživatelský komfort (přihlašování otiskem prstu či obličejem přímo v~prohlížeči) s~absolutní nezávislostí na jakýchkoliv externích korporátních rozhraních a~vývojářských portálech. V~oblasti distribuce obsahu a~koordinace spolujízd se jako přirozená evoluční cesta nabízí hlubší integrace do komunitních platforem Telegram a~Discord, které byly v~práci vyhodnoceny jako velmi perspektivní. Vývoj dedikovaných komunikačních botů a~přímých upozornění o~obsazenosti vozidel, případně transformace klientského rozhraní do podoby interní chatové aplikace, by umožnily koordinovat spolujízdu a~zobrazovat interaktivní schéma vozu přímo v~kontextu probíhající skupinové konverzace, což by posunulo uživatelskou přívětivost celého systému na novou úroveň.
...
Externí přílohy této bakalářské práce jsou umístěny na adrese:\\ \url{https://github.com/matthewjezek/thesis_ki_ujep}.
