...
Práce je strukturována do šesti logicky navazujících kapitol. První kapitola definuje teoretická východiska autorizačních standardů na webu. Druhá kapitola přináší rešerši existujících řešení, na kterou navazuje třetí kapitola analyzující integrační rozhraní vybraných platforem a s nimi spojená rizika. Čtvrtá kapitola obsahuje komplexní inženýrský návrh architektury a datového modelu aplikace Sitzy. Pátá kapitola detailně popisuje praktickou realizaci, zajištění kvality a nasazení celého systému. Šestá kapitola se následně věnuje uživatelskému výzkumu a vyhodnocení telemetrických dat, přičemž celkové dosažené výsledky a reflexi řešení shrnuje Závěr.

\chapter{Teoretická východiska autorizace}

Při návrhu integrace vlastních webových služeb do prostředí sociálních sítí je klíčové nejprve definovat, jakou roli tyto platformy v~širším ekosystému hrají. Jak uvádí Quesenberry, z~pohledu ucelené strategie již sociální sítě nepředstavují pouze izolovaná komunikační rozhraní, ale staly se nezbytnou součástí budování vlastněných (\textit{owned}) a sdílených (\textit{shared}) mediálních kanálů \cite{Quesenberry2024}. Technicky tak tvoří komplexní infrastrukturu, jejíž integrační potenciál a služby poskytované externím aplikacím lze rozdělit do tří základních vrstev:

\begin{enumerate}
    \item \textbf{Autentizační a identitní vrstva (Federated Identity):} Umožňuje přenést správu identit na zavedené poskytovatele (např. Facebook, X). Pro externí službu to znamená eliminaci nutnosti implementovat a zabezpečovat vlastní databázi hesel, čímž se radikálně snižují nároky na uživatele při registraci.
